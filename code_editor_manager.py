...
class CodeEditorManager:
    ...
    def get_selected_text(self):
        if not self.code_editor:
            return ""
        try:
            return self.code_editor.get(tk.SEL_FIRST, tk.SEL_LAST)
        except tk.TclError:
            return ""
    ...
    def get_current_content(self):
        if not self.code_editor:
            return ""
        return self.code_editor.get('1.0', tk.END + '-1c')
    ...
    def replace_selection(self, new_text: str):
        ...
        try:
            ...
            self.code_editor.insert(tk.INSERT, new_text)
            self.on_editor_content_modified()
            return True
        except tk.TclError:
            return False

    def replace_all_content(self, new_text: str):
        if not self.code_editor:
            return False
        self.code_editor.delete('1.0', tk.END)
        self.code_editor.insert('1.0', new_text)
        self.on_editor_content_modified()
        return True

    def replace_selection_or_all(self, new_text: str):
        if self.replace_selection(new_text):
            return 'selection'
        if self.replace_all_content(new_text):
            return 'document'
        return None
    ...
    def get_selection_indices(self):
        if not self.code_editor:
            return None
        try:
            return (self.code_editor.index(tk.SEL_FIRST), self.code_editor.index(tk.SEL_LAST))
        except tk.TclError:
            return None
    ...
    def get_current_line_text(self):
        ...
        try:
            ...
            return self.code_editor.get(line_start, line_end)
        except Exception:
            return ''
