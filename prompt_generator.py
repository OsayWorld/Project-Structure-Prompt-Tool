...
class PromptGenerator:
    ...
    def set_prompt_content(self, content: str, status_message: str = "Ready"):
        self._update_prompt_text_area(content or "", status_message)

    def paste_clipboard_to_prompt(self):
        try:
            ...
            if not clipboard_text:
                messagebox.showwarning("Clipboard Empty", "No text found in clipboard.", parent=self.app)
                return
            self.set_prompt_content(clipboard_text, "📥 Clipboard pasted into AI panel.")
        except Exception as e:
            messagebox.showerror("Paste Failed", f"Could not paste clipboard text: {e}", parent=self.app)

    def _get_current_file_path(self):
        manager = getattr(self.app, 'code_editor_manager', None)
        return getattr(manager, 'current_editor_file', None)

    def _read_text_file(self, file_path: str, max_chars: int = 16000) -> str:
        ...
        try:
            ...
            if max_chars and len(content) > max_chars:
                return content[:max_chars] + "\n... (truncated for AI workspace)"
            return content
        except Exception:
            return ""

    def _get_language_for_file(self, file_path: str, content_sample: str = "") -> str:
        try:
            lexer = self._get_pygments_lexer(file_path or 'snippet.txt', content_sample[:1024])
            name = lexer.name.lower().replace('lexer', '').strip()
            return name or 'text'
        except Exception:
            return 'text'
    ...
    def _project_summary_block(self) -> str:
        path = getattr(self.project_scanner, 'project_path', None)
        if not path:
            return "No project loaded."
        frameworks = ', '.join(self.detect_frameworks()) or 'No framework signals detected.'
        files = getattr(self.project_scanner, 'file_count', 0)
        size = self.project_scanner.format_size(getattr(self.project_scanner, 'total_size', 0)) if hasattr(self.project_scanner, 'format_size') else 'Unknown'
        rel = os.path.basename(path)
        return f"Project: {rel}\nFiles: {files}\nSize: {size}\nFrameworks: {frameworks}"

    def generate_explain_file_prompt(self, copy_to_clipboard: bool = True):
        file_path = self._get_current_file_path()
        if not file_path or not os.path.isfile(file_path):
            messagebox.showwarning("No File Open", "Open a file in the editor first.", parent=self.app)
            return
        content = self._read_text_file(file_path, max_chars=max(self.config_manager.max_prompt_file_length, 12000))
        if not content:
            messagebox.showwarning("Read Failed", "Could not read the current file.", parent=self.app)
            return
        rel_path = os.path.relpath(file_path, self.project_scanner.project_path) if self.project_scanner.project_path else file_path
        language = self._get_language_for_file(file_path, content)
        prompt = f"""🧠 EXPLAIN FILE
{'=' * 60}

You are analyzing one file from a local codebase.

PROJECT SNAPSHOT
{self._project_summary_block()}

TASK
Explain this file in a way that helps a working developer move faster.

Please provide:
1. File purpose
2. Main classes, functions, and responsibilities
3. Inputs, outputs, side effects, and dependencies
4. Risks, bugs, or maintainability concerns
5. Concrete improvement suggestions
6. A short 'what to change first' section

Response style:
- Be practical
- Use headings
- Quote exact symbols when relevant
- Keep it grounded in the file only unless clearly inferred
...
```
"""
        self.set_prompt_content(prompt, f"🧠 Explain prompt ready for {os.path.basename(file_path)}")
        try:
            self.ui_builder.right_notebook.select(1)
        except Exception:
            pass
        if copy_to_clipboard and self._copy_text_to_clipboard(prompt):
            self.app.set_status("🧠 Explain prompt copied to clipboard.")

    def _build_inline_edit_prompt(self, action_name: str, instruction: str, selection_text: str, file_path: str, full_content: str) -> str:
        rel_path = os.path.relpath(file_path, self.project_scanner.project_path) if self.project_scanner.project_path and file_path else (file_path or 'Current editor buffer')
        language = self._get_language_for_file(file_path or 'snippet.txt', selection_text or full_content)
        surrounding = full_content[:12000]
        if len(full_content) > 12000:
            surrounding += "\n... (truncated file context)"
        return f"""✨ INLINE AI EDIT REQUEST
{'=' * 60}
...
Return ONLY the revised code for the target snippet inside one fenced code block.
Do not include explanation before the code.
After the code block, include a short bullet list named CHANGES with the reasoning.
...
"""
    ...
    def generate_inline_ai_edit_prompt(self, action_name: str, default_instruction: str, copy_to_clipboard: bool = True):
        manager = getattr(self.app, 'code_editor_manager', None)
        file_path = self._get_current_file_path()
        if not manager or not file_path or not os.path.isfile(file_path):
            messagebox.showwarning("No File Open", "Open a file in the editor first.", parent=self.app)
            return
        selection_text = manager.get_selected_text().strip()
        if not selection_text:
            messagebox.showwarning("No Selection", "Select the code you want the AI to rewrite first.", parent=self.app)
            return
        instruction = simpledialog.askstring(
            f"{action_name}",
            "Add any extra instruction for the AI.\nLeave as-is to use the default action.",
            initialvalue=default_instruction,
            parent=self.app,
        )
        if instruction is None:
            self.app.set_status("AI edit cancelled.")
            return
        full_content = manager.get_current_content()
        prompt = self._build_inline_edit_prompt(action_name, instruction.strip() or default_instruction, selection_text, file_path, full_content)
        self.set_prompt_content(prompt, f"✨ {action_name} prompt ready.")
        try:
            self.ui_builder.right_notebook.select(1)
        except Exception:
            pass
        if copy_to_clipboard and self._copy_text_to_clipboard(prompt):
            self.app.set_status(f"✨ {action_name} prompt copied to clipboard.")

    def generate_custom_ai_edit_prompt(self):
        instruction = simpledialog.askstring(
            "Custom AI Instruction",
            "What should the AI do with the selected code?",
            parent=self.app,
        )
        if instruction is None:
            return
        ...
        if not instruction:
            messagebox.showwarning("Instruction Required", "Enter a task for the AI.", parent=self.app)
            return
        self.generate_inline_ai_edit_prompt("Custom Rewrite", instruction, copy_to_clipboard=True)
    ...
    def apply_ai_response_to_editor(self):
        manager = getattr(self.app, 'code_editor_manager', None)
        if not manager or not self.prompt_text_widget:
            return
        raw = self.prompt_text_widget.get('1.0', tk.END).strip()
        if not raw:
            messagebox.showwarning("No AI Response", "Paste the AI response into the prompt panel first.", parent=self.app)
            return
        code = self._extract_code_block(raw)
        if not code:
            messagebox.showwarning("No Code Found", "Could not find code to apply.", parent=self.app)
            return
        if manager.has_selection():
            ok = messagebox.askyesno("Apply AI Response", "Replace the selected code with the code found in the AI response?", parent=self.app)
        else:
            ok = messagebox.askyesno("Apply AI Response", "No editor selection found. Replace the entire editor content with the AI response code?", parent=self.app)
        if not ok:
            return
        replaced = manager.replace_selection_or_all(code)
        if not replaced:
            messagebox.showerror("Apply Failed", "Could not apply the AI response to the editor.", parent=self.app)
            return
        self.app.set_status(f"✅ Applied AI response to {replaced}.")
    ...
    def copy_checked_items(self):
        """Copies only the files that are currently checked (☑) in the tree."""
        if not hasattr(self.project_scanner, 'project_path') or not self.project_scanner.project_path:
            messagebox.showwarning("No Project", "Please load a project first.", parent=self.app)
            return

        # Use the improved scanner logic to get only checked files
        checked_files = self.project_scanner.get_all_files(respect_exclusions=True)

        if not checked_files:
            messagebox.showinfo("No Selection", "No files are currently checked (☑).", parent=self.app)
            return

        # If it's a lot of files, warn the user
        if len(checked_files) > 50:
            if not messagebox.askyesno("Large Copy", f"You have {len(checked_files)} files checked. Copy all of them?"):
                return

        self.app.set_status(f"Generating prompt for {len(checked_files)} checked files...")
        self.generate_prompt(checked_files, post_generation_callback=self.copy_current_prompt)
    ...
    def copy_full_project_code(self):
        """
        Copy the entire project code without truncation.
        Includes project structure and full content of all eligible files.
        Respects folder and file exclusions set by the user.
        """
        all_files_paths = self._prepare_full_project_export("copy")
        ...
        self.full_copy_thread = threading.Thread(
            target=self._generate_and_copy_full_project_content_worker,
            args=(all_files_paths,),
            daemon=True,
        )
        self.full_copy_thread.start()

    def export_full_project_code_to_txt(self):
        """Save a .txt clone of the same full-project content used by Copy Full Project."""
        all_files_paths = self._prepare_full_project_export("save")
        if not all_files_paths:
            return

        self.app.set_status("⬇️ Preparing TXT clone of full project code...")

        if self.full_copy_thread and self.full_copy_thread.is_alive():
            self.app.set_status("Another full project export is already in progress. Please wait.")
            return

        self.full_copy_thread = threading.Thread(
            target=self._generate_and_save_full_project_content_worker,
            args=(all_files_paths,),
            daemon=True,
        )
        self.full_copy_thread.start()

    def _prepare_full_project_export(self, action_label="copy"):
        if not hasattr(self.project_scanner, 'project_path') or not self.project_scanner.project_path:
            messagebox.showwarning("No Project", "Please load a project first.", parent=self.app)
            return None
        ...
        if not all_files_paths:
            messagebox.showinfo("No Files", "No eligible files found in the project. All items may be excluded.", parent=self.app)
            return None

        excluded_folder_count = len(getattr(self.project_scanner, 'excluded_folders', set()))
        excluded_file_count = len(getattr(self.project_scanner, 'excluded_files', set()))
        warning_message = ""
        if excluded_folder_count > 0 or excluded_file_count > 0:
            warning_message += (
                f"Note: {excluded_folder_count} folder(s) and {excluded_file_count} file(s) "
                f"are excluded and will not be included.\n\n"
            )

        if total_content_size > self.FULL_COPY_SIZE_WARNING_THRESHOLD:
            warning_message += (
                f"The total size of project files ({self.project_scanner.format_size(total_content_size)}) exceeds the recommended limit "
                f"of {self.project_scanner.format_size(self.FULL_COPY_SIZE_WARNING_THRESHOLD)}.\n"
            )
        if len(all_files_paths) > self.FULL_COPY_FILE_COUNT_WARNING_THRESHOLD:
            warning_message += f"The project contains {len(all_files_paths)} files, which may take a while to process.\n"

        if warning_message:
            verb = "copy" if action_label == "copy" else "save"
            warning_message += f"\nProceed and {verb} the full project content?"
            if not messagebox.askyesno("Large Project Export", warning_message, parent=self.app):
                return None

        return all_files_paths

    def _build_full_project_content(self, all_files_paths):
        ...
        for file_path in all_files_paths:
            try:
                if not os.path.exists(file_path) or os.path.isdir(file_path):
                    output_buffer.write(
                        f"### FILE: {os.path.relpath(file_path, self.project_scanner.project_path)} "
                        f"(Skipped: Not a valid file or does not exist)\n\n"
                    )
                    continue
    ...
    def _generate_and_copy_full_project_content_worker(self, all_files_paths):
        ...
        try:
            full_content = self._build_full_project_content(all_files_paths)
    ...
    def _generate_and_save_full_project_content_worker(self, all_files_paths):
        """Worker thread to generate the full project content string and save it to a .txt file."""
        try:
            full_content = self._build_full_project_content(all_files_paths)
            self.app.after(0, self._finalize_full_project_save, full_content)
        except Exception as e:
            self.app.after(0, lambda: messagebox.showerror("Error", f"Failed to generate TXT clone: {e}", parent=self.app))
            self.app.after(0, lambda: self.app.set_status("❌ Error generating TXT clone."))
    ...
    def _finalize_full_project_save(self, full_content):
        """Prompt for a destination and save the generated full project content as a text file."""
        filename = filedialog.asksaveasfilename(
            title="Save TXT Clone of Full Project",
            defaultextension=".txt",
            initialfile="full_project_code_clone.txt",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")],
            parent=self.app,
        )
        if not filename:
            self.app.set_status("TXT clone export cancelled.")
            return

        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(full_content)
            self.app.set_status(f"⬇️ TXT clone saved to {os.path.basename(filename)}")
            messagebox.showinfo("TXT Clone Saved", f"✅ TXT clone successfully saved to:\n{filename}", parent=self.app)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save TXT clone: {e}", parent=self.app)
            self.app.set_status("❌ Error saving TXT clone.")
