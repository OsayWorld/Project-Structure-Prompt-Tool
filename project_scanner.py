...
class ProjectScanner:
    def __init__(self, app_instance, config_manager):
        ...
        class _WorkspaceState:
            ...
            excluded_files: set = field(default_factory=set)
    ...
    def prompt_for_project(self):
        """Open a stable native folder picker across platforms."""
        try:
            initial_dir = self.active_workspace_path or self.project_path or self.config_manager.last_project_path or os.path.expanduser("~")
            ...
            if project_path and os.path.exists(project_path):
                self.add_workspace(os.path.abspath(project_path))
            else:
                self.app.set_status("No folder selected.")
        except Exception as e:
            import logging
            logging.getLogger(__name__).exception("Folder selection failed")
            self.app.set_status(f"Error selecting folder: {e}")

    def _get_folder_windows_safe(self):
        """Legacy Windows folder picker retained only as a fallback helper."""
        try:
            # This is a robust way to open the folder picker without using Tkinter's buggy wrapper
            from ctypes import windll, CFUNCTYPE, c_wchar_p, c_int, Structure, POINTER
            
            # Define BROWSEINFO structure for SHBrowseForFolder
            ...
            # BIF_NEWDIALOGSTYLE = 0x00000040 (Modern resizable dialog)
            # BIF_RETURNONLYFSDIRS = 0x0001
            # BIF_USENEWUI = 0x0040 | 0x0010
            ...
            bi = BROWSEINFO()
            bi.hwndOwner = self.app.winfo_id()
            ...
            if pidl:
                ...
                if shell32.SHGetPathFromIDListW(pidl, path):
                    ...
                    windll.shell32.ILFree(pidl)
                    return path.value
            return None
        except Exception as e:
            print(f"Native dialog failed, falling back to basic: {e}")
    ...
    def _set_editor_context(self, file_path=None):
        try:
            if hasattr(self.ui_builder, "set_current_file"):
                self.ui_builder.set_current_file(file_path)
        except Exception:
            pass

    def populate_file_list(self, file_items):
        """Populate the active file list with the provided scanned file tuples."""
        if not self.ui_builder.file_list:
            return
        try:
            for item in self.ui_builder.file_list.get_children():
                self.ui_builder.file_list.delete(item)
            for icon_rel_path, size_str, ext_upper, full_path in file_items:
                self.ui_builder.file_list.insert("", "end", text=icon_rel_path, values=[size_str, ext_upper, full_path])
        except Exception as e:
            self.app.set_status(f"Error updating file list: {e}")

    def show_all_files(self):
        """Show all files for the active workspace, respecting search if present."""
        self._do_search()

    def show_files_for_folder(self, folder_path):
        """Show only files inside the selected folder subtree."""
        if not self.ui_builder.file_list:
            return
        folder_path = os.path.abspath(folder_path)
        filtered = []
        for item_data in self.scanned_file_list_data:
            full_path = os.path.abspath(item_data[3])
            if full_path.startswith(folder_path + os.sep) or full_path == folder_path:
                filtered.append(item_data)
        self.populate_file_list(filtered)

    def select_file_in_tree(self, file_path):
        """Focus the matching tree item for a selected file list row."""
        if not self.ui_builder.tree or not file_path:
            return
        file_path = os.path.abspath(file_path)
        try:
            def walk(parent=""):
                for item_id in self.ui_builder.tree.get_children(parent):
                    values = self.ui_builder.tree.item(item_id, "values")
                    if values and os.path.abspath(values[0]) == file_path:
                        return item_id
                    found = walk(item_id)
                    if found:
                        return found
            ...
            found = walk("")
            if found:
                parent = self.ui_builder.tree.parent(found)
                while parent:
                    self.ui_builder.tree.item(parent, open=True)
                    parent = self.ui_builder.tree.parent(parent)
                self.ui_builder.tree.selection_set(found)
                self.ui_builder.tree.focus(found)
                self.ui_builder.tree.see(found)
        except Exception:
            pass
            ...
            if selection:
                ...
                if values and len(values) > 0:
                    ...
                    if os.path.isfile(file_path):
                        ...
                        self._set_editor_context(file_path)
                        self.prompt_generator.generate_prompt([file_path])
                        self.show_all_files()
                        ...
                        self.code_editor_manager.clear_editor_highlight()
                        self.code_editor_manager.update_line_numbers()
                        self._set_editor_context(None)
                        self.prompt_generator.clear_prompt_text()
                        self.show_files_for_folder(file_path)
    ...
    def is_path_excluded(self, path):
        """Check if a file or folder is excluded, either directly or by parent."""
        path = os.path.abspath(path)
        
        # 1. Check direct file exclusion
        if path in self.excluded_files:
            return True
            
        # 2. Check if any parent directory is in excluded_folders
        # We walk up the directory tree until we hit the project root
        current = path
        while True:
            if current in self.excluded_folders:
                return True
            parent = os.path.dirname(current)
            # Stop if we've reached the root or are no longer inside the project
            if parent == current or not current.startswith(self.project_path):
                break
            current = parent
    ...
    def get_all_files(self, respect_exclusions=False):
        """Get all eligible project file paths."""
        if not self.project_path:
            return []
        ...
        # Filter based on the checkboxes/exclusion sets
        return [f for f in all_files if not self.is_path_excluded(f)]
    ...
    def clear_index_cache(self):
        """Delete the .project_index_cache folder and recreate it."""
        if not messagebox.askyesno("Clear Cache", "Are you sure you want to delete all cached project indexes?\nThis will force a slow rescan for all workspaces next time they are opened.", parent=self.app):
            return
        ...
        try:
            if os.path.exists(self.index_cache_dir):
                ...
                os.makedirs(self.index_cache_dir, exist_ok=True)
                self.app.set_status("🗑️ Cache cleared successfully.")
                messagebox.showinfo("Success", "Project index cache has been cleared.", parent=self.app)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to clear cache: {e}", parent=self.app)
    ...
    def open_in_explorer(self, path):
        ...
        try:
            ...
            if not os.path.exists(path):
                self.app.set_status("⚠️ Path does not exist.")
                return

            if platform.system() == "Windows":
                # For files, this opens the folder and highlights the file
                if os.path.isfile(path):
                    subprocess.run(['explorer', '/select,', path])
                else:
                    os.startfile(path)
            elif platform.system() == "Darwin":  # macOS
                subprocess.run(["open", "-R" if os.path.isfile(path) else "", path])
            else:  # Linux
                subprocess.run(["xdg-open", os.path.dirname(path) if os.path.isfile(path) else path])
            
            self.app.set_status(f"📂 Opened in Explorer: {os.path.basename(path)}")
        except Exception as e:
            messagebox.showerror("Error", f"Could not open explorer: {e}", parent=self.app)